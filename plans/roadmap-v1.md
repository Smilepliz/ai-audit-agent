...
### Будущее: Content Agent (ориентировочно после v1.0)

**Статус:** Идея, проработка

Content Agent — отдельный модуль, который использует результаты аудита сайта для поиска контентных возможностей. Не является обязательной частью SiteAuditBot и разрабатывается как самостоятельный инструмент или расширение.

**Целевой функционал:**
- Анализ наличия и структуры блога на сайте
- Выявление тематических пробелов (content gaps)
- Генерация идей и тем для статей на основе аудита
- Рекомендации по FAQ, кейсам, гайдам и экспертному контенту
- Подготовка структурированных задач (JSON) для будущего контент-агента

**Зависимости:** Стабилизированный аудитор (v1.0+), модуль JSON-отчёта
...
### 2.5 Content Agent (post-1.0, отдельный модуль)

- **Польза:** Высокая — превращает аудит данных в практические рекомендации по контенту.
- **Сложность:** Высокая — нужен LLM-анализ контента, классификация тем и аккуратная структура рекомендаций.
- **Приоритет:** Низкий (post-1.0, после стабилизации аудитора).

Что даёт:
- От аудита "что сломано" к аудиту "чего не хватает".
- JSON-выгрузка задач для внешнего контент-агента.
...
| Content Agent | Высокая | Высокая | Низкий | post-1.0 | 💡 |
